...
export default function ConfigPanel({ config, onChange }: ConfigPanelProps) {
  ...
  const updateWarehouse = (idx: number, patch: Partial<WarehouseUIState>) => {
    const updated = config.warehouseStates.map((w, i) => {
      if (i !== idx) return w;
      return { ...w, ...patch };
    });

    // If Matupá changed and Alta Floresta not manual, recalculate
    ...
    if (idx === matupaIdx && afIdx >= 0) {
      ...
      if (!af.basisSojaManual && patch.basisSoja !== undefined) {
        updated[afIdx] = { ...af, basisSoja: updated[matupaIdx].basisSoja - 1.0 };
      }
      if (!af.basisMilhoManual && patch.basisMilho !== undefined) {
        updated[afIdx] = { ...updated[afIdx], basisMilho: updated[matupaIdx].basisMilho - 1.5 };
      }
    }

    // If Alta Floresta edited manually, flag it
    if (idx === afIdx) {
  ...
  const removeDateSoja = (d: string) => {
    onChange({ ...config, paymentDatesSoja: config.paymentDatesSoja.filter((x) => x !== d) });
  };
  ...
  const removeDateMilho = (d: string) => {
    onChange({ ...config, paymentDatesMilho: config.paymentDatesMilho.filter((x) => x !== d) });
  };
      ...
      <div className="rounded-lg border bg-card">
        ...
        <CollapsibleContent>
          <div className="space-y-4 border-t px-4 py-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  ...
                  <Input type="number" step="0.01" value={config.sharedCosts.interestRate} onChange={(e) => updateCost("interestRate", parseFloat(e.target.value) || 0)} className="h-8 font-mono text-sm" />
                </div>
                <div className="space-y-1">
                ...
                <div className="space-y-1">
                  ...
                  <Input type="number" step="0.01" value={config.sharedCosts.storageCost} onChange={(e) => updateCost("storageCost", parseFloat(e.target.value) || 0)} className="h-8 font-mono text-sm" />
                </div>
                <div className="space-y-1">
                ...
                <div className="space-y-1">
                  ...
                  <Input type="number" step="0.01" value={config.sharedCosts.receptionCost} onChange={(e) => updateCost("receptionCost", parseFloat(e.target.value) || 0)} className="h-8 font-mono text-sm" />
                </div>
                <div className="space-y-1">
                  <Label className="text-xs text-muted-foreground">Corretagem CBOT (USD/ct)</Label>
                  <Input type="number" step="0.01" value={config.sharedCosts.brokeragePerContract} onChange={(e) => updateCost("brokeragePerContract", parseFloat(e.target.value) || 0)} className="h-8 font-mono text-sm" />
                </div>
                <div className="space-y-1">
                  <Label className="text-xs text-muted-foreground">Desk cost (%)</Label>
                  <Input type="number" step="0.001" value={config.sharedCosts.deskCostPct} onChange={(e) => updateCost("deskCostPct", parseFloat(e.target.value) || 0)} className="h-8 font-mono text-sm" />
                </div>
                <div className="space-y-1">
                  <Label className="text-xs text-muted-foreground">Quebra técnica (%/mês)</Label>
                  <Input type="number" step="0.001" value={config.sharedCosts.shrinkageRateMonthly} onChange={(e) => updateCost("shrinkageRateMonthly", parseFloat(e.target.value) || 0)} className="h-8 font-mono text-sm" />
                </div>
              </div>
            </div>
              ...
              <div className="grid gap-3 sm:grid-cols-3">
                {config.warehouseStates.map((wh, idx) => (
                  <div key={wh.id} className="rounded-md border p-3 space-y-2">
                    <p className="text-xs font-semibold">{wh.displayName}</p>
                    <div className="space-y-1">
                      <Label className="text-[10px] text-muted-foreground">Basis Soja (R$/sc)</Label>
                      <Input type="number" step="0.5" value={wh.basisSoja} onChange={(e) => updateWarehouse(idx, { basisSoja: parseFloat(e.target.value) || 0 })} className="h-7 font-mono text-xs" />
                    </div>
                    <div className="space-y-1">
                      <Label className="text-[10px] text-muted-foreground">Basis Milho (R$/sc)</Label>
                      <Input type="number" step="0.5" value={wh.basisMilho} onChange={(e) => updateWarehouse(idx, { basisMilho: parseFloat(e.target.value) || 0 })} className="h-7 font-mono text-xs" />
                    </div>
                    <div className="space-y-1">
                      <Label className="text-[10px] text-muted-foreground">Corretagem B3 (BRL/ct)</Label>
                      <Input type="number" step="0.5" value={wh.brokMilho} onChange={(e) => updateWarehouse(idx, { brokMilho: parseFloat(e.target.value) || 0 })} className="h-7 font-mono text-xs" />
                    </div>
                    <div className="space-y-1">
                      <Label className="text-[10px] text-muted-foreground">Quebra milho (%)</Label>
                      <Input type="number" step="0.001" value={wh.shrinkMilho} onChange={(e) => updateWarehouse(idx, { shrinkMilho: parseFloat(e.target.value) || 0 })} className="h-7 font-mono text-xs" />
                    </div>
                  </div>
                ))}
              </div>
            ...
            {/* BLOCO 3 — Datas de Pagamento */}
            <div>
              <h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                Datas de Pagamento Extras
              </h3>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {config.paymentDatesSoja.map((d) => (
                      <span key={d} className="inline-flex items-center gap-1 rounded-full border bg-muted px-2 py-0.5 text-xs font-mono">
                        {d}
                        <button onClick={() => removeDateSoja(d)} className="text-muted-foreground hover:text-destructive">
                          <X className="h-3 w-3" />
                        </button>
                      </span>
                    ))}
                  ...
                  <div className="flex gap-1.5">
                    <Input type="date" value={newDateSoja} onChange={(e) => setNewDateSoja(e.target.value)} className="h-7 text-xs flex-1" />
                    <Button size="sm" variant="outline" className="h-7 px-2" onClick={addDateSoja}>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {config.paymentDatesMilho.map((d) => (
                      <span key={d} className="inline-flex items-center gap-1 rounded-full border bg-muted px-2 py-0.5 text-xs font-mono">
                        {d}
                        <button onClick={() => removeDateMilho(d)} className="text-muted-foreground hover:text-destructive">
                          <X className="h-3 w-3" />
                        </button>
                      </span>
                    ))}
                  ...
                  <div className="flex gap-1.5">
                    <Input type="date" value={newDateMilho} onChange={(e) => setNewDateMilho(e.target.value)} className="h-7 text-xs flex-1" />
                    <Button size="sm" variant="outline" className="h-7 px-2" onClick={addDateMilho}>
                      <Plus className="h-3 w-3" />
                    </Button>
                  </div>
                </div>
